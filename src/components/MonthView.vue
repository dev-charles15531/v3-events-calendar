...
import { computed, ComputedRef, onBeforeUnmount, watch } from "vue";
...
const emit = defineEmits(["togglePopover", "cleanPopover", "openModal"]);
const { daysOfWeekArr, handlePopoverToggle, handleModalOpen } = useUtils(emit);
...
<template>
  <!-- <div class="flex-grow"> -->
  <div
    class="lg:h-full shadow ring-1 ring-gray-300 ring-opacity-5 lg:flex lg:flex-auto lg:flex-col"
  >
    <div
      class="grid grid-cols-7 gap-px border-b border-gray-300 bg-gray-200 text-center text-xs font-semibold leading-6 text-gray-700 lg:flex-none"
    >
      <div
        v-for="day in daysOfWeekArr(currentDate, sundayStartWeek)"
        :key="day"
        class="bg-white py-2"
      >
        {{ day }}
